...
// a constructor taking an array of characters  and the integer length of the array, and creates an array list containing the characters of the array with an initial capacity equal to the length of the array that was passed in.
...
//replaceAt function that takes an element(char)and an integer index as parameters and has a void return type. It replaces the element at the specified index with the new element. If the index is out of range it should throw a std::range_error exception with message “index (IDX) not in range [0..SIZE)”,
